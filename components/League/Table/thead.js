...
export default function thead({setToHover, toHover, ShouldSticky: isSticky, NavbarHeight}) {
    ...
    return (
        <tr style={isSticky ? StickyStyle : {}} onScroll={() => {}} className="row100 head sticky-inner">
            <th onMouseOver={() => setToHover("c1")} onMouseOut={() => setToHover("")} className={`column100 column1 ${toHover === "c1" ? " hov-column-head-ver1" : ""}`}>Card Name</th>
